...
dumpbin /headers release\%NMAKE_PLATFORM%\pkg-config.exe > pkg-config-headers.txt
if errorlevel 1 exit 1
type pkg-config-headers.txt
findstr /I /C:"machine (%NMAKE_PLATFORM%)" pkg-config-headers.txt
if errorlevel 1 exit 1
